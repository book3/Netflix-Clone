...
//  Netflix Clone
//
//  Created by Yntymak on 19/10/22.
//
    ...
    private func configureHeroHeaderView(){
        APICaller.shared.getTrendingMovies { [weak self] results in
            ...
            case .success(let titles):
                ...
                self?.headerView?.configure(with: TitleViewModel(titleName: selectedTitle?.original_title ?? selectedTitle?.original_name ?? "Undefined name", posterURL: selectedTitle?.poster_path ?? ""))
...
extension HomeViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, willDisplayHeaderView view: UIView, forSection section: Int) {
        ...
        header.textLabel?.text = header.textLabel?.text?.capitalizeFirsLetter()
    }
